...
function ModifierStack({
  ...
  function remove(id: string) {
    onChange(stack.filter((m) => m.id !== id));
  }
  function updateSource(id: string, source: string) {
    onChange(stack.map((m) => (m.id === id ? { ...m, source } : m)));
  }
  function updateValue(id: string, value: number) {
    onChange(stack.map((m) => (m.id === id ? { ...m, value } : m)));
  }
...
export function SavesBlock({
  ...
  function saveCalc(attr: AttributeKey) {
    ...
    const saveStack = saves[attr].stack
      .filter((m) => m.isActive)
      .reduce((s, m) => s + m.value, 0);
    ...
    return (
      attrMod +
      (saves[attr].proficient ? proficiencyBonus : 0) +
      saveStack +
      globalSum
    );
  }
  ...
  function handleOverrideChange(attr: AttributeKey, raw: string) {
    if (raw === "") {
      onSaveOverrideChange(attr, null);
      return;
    }
    const n = parseInt(raw, 10);
  ...
  function toggleProficiency(attr: AttributeKey) {
    ...
    if (save.override !== null)
      onSaveOverrideChange(attr, save.override + delta);
  }
          ...
          <div key={attr} className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                aria-label={
                  save.proficient ? "Remove proficiency" : "Add proficiency"
                }
              ...
              <div className="relative min-w-0 flex-1">
                ...
                {isOverridden && (
                  <button
                    type="button"
                    aria-label="Reset"
                    onClick={() => onSaveOverrideChange(attr, null)}
