...
export function StatBlock({ label, data, onBaseChange, onStackChange, onOverrideChange }: StatBlockProps) {
  ...
  function handleBaseChange(raw: string) {
    if (raw === "") return
    const n = parseInt(raw, 10)
    if (!isNaN(n)) onBaseChange(Math.min(30, Math.max(1, n)))
  }
        ...
        <div className="relative min-w-0 flex-1">
          <input
            ...
            className={cn(
              ...
              data.override !== null && "pr-5",
            )}
          />
          {data.override !== null && (
            <button type="button" aria-label="Reset to calculated value" onClick={() => onOverrideChange(null)}
