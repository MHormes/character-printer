...
export function ForgeSection({ 
  ...
  const storageKey = charId ? `forge-section-collapsed:${charId}:${title}` : null
  ...
  return (
    <section className={cn("space-y-3 rounded-xl border border-border bg-section p-4", className)}>
      <div className="flex items-center justify-between gap-3">
        ...
        >
          {collapsible && (
            isCollapsed ? <ChevronDown className="size-3 text-muted-foreground" /> : <ChevronUp className="size-3 text-muted-foreground" />
          )}
