...
import type { AttributeKey, AttributeData, SkillData, SkillState, ModifierEntry, DerivedValueData } from "@/lib/types/character"

import { resolveSkillBonus, resolvePb, resolvePassivePerception } from "@/lib/character/calculations"
...
export function SkillsBlock({
  ...
  const passiveSum = passivePerception.stack.filter((m) => m.isActive).reduce((s, m) => s + m.value, 0)
  ...
  function removePassiveMod(id: string) {
    onPassivePerceptionStackChange(passivePerception.stack.filter((m) => m.id !== id))
  }

  function updatePassiveMod(id: string, patch: Partial<ModifierEntry>) {
    onPassivePerceptionStackChange(
      passivePerception.stack.map((m) => (m.id === id ? { ...m, ...patch } : m)),
    )
  }

  function handlePassiveOverrideChange(raw: string) {
    if (raw === "") { onPassivePerceptionOverrideChange(null); return }
    if (raw === "-") return
    if (!/^-?\d+$/.test(raw)) return
    const n = parseInt(raw, 10)
                ...
                <div key={mod.id} className={cn("flex items-start gap-1", !mod.isActive && "opacity-40")}>
                  <div className="flex min-w-0 flex-1 flex-col gap-1">
                    <Input
                      ...
                      onChange={(e) => updatePassiveMod(mod.id, { source: e.target.value })}
                    />
                    <div className="flex h-6 items-center rounded-md border border-input bg-background focus-within:border-ring">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (raw === "") { updatePassiveMod(mod.id, { value: 0 }); return }
                          if (raw === "-") return
                          const n = parseInt(raw, 10)
                  ...
                  <div className="mt-0.5 flex flex-col gap-0.5">
                    ...
                    <button
                      type="button"
                      onClick={() => updatePassiveMod(mod.id, { isActive: !mod.isActive })}
