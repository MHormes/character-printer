...
export function ClassesField({
  ...
  function update<K extends keyof CharacterClassEntry>(
    index: number,
    key: K,
    value: CharacterClassEntry[K],
  ) {
  ...
  function pickSubclassFromDb(index: number, dbSubclass: SubclassRow) {
    onChange(
      classes.map((c, i) =>
        i === index
          ? { ...c, subclass: dbSubclass.name, subclassId: dbSubclass.id }
          : c,
      ),
    );
    setSubclassOpenIdx(null);
    ...
    <div className="flex flex-col gap-0" ref={containerRef}>
      ...
      {classes.map((cls, i) => {
        ...
        const filteredSubclasses = availableSubclasses.filter(
          (s) =>
            ...
            s.name.toLowerCase().includes(cls.subclass.toLowerCase()),
        );
        const hasDbSubclasses =
          cls.classId !== null && filteredSubclasses.length > 0;
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      onChange(
                        classes.map((c, idx) =>
                          idx === i
                            ? { ...c, classId: null, name: e.target.value }
                            : c,
                        ),
                      );
                      setOpenIdx(i);
              ...
              <div className="relative">
                <div className="flex h-6 items-center rounded-md border border-input bg-background shadow-sm focus-within:border-ring focus-within:ring-3 focus-within:ring-ring/50">
                  <input
                    ...
                    onFocus={() => {
                      if (hasDbSubclasses) setSubclassOpenIdx(i);
                    }}
                  />
                  {cls.subclassId && (
                    <button
      ...
      <div className="mt-1 flex items-center gap-3">
        ...
        <div className="flex h-8 items-center gap-1 rounded-md border border-border bg-muted px-3">
          <span className="text-xs font-medium uppercase tracking-wide text-card-foreground/60">
            PB
          </span>
