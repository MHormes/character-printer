...
export function SpellCardSvg({ spell }: { spell: SpellEntry }) {
  const CW = 180, CH = 252, PAD = 8, MX = 90
  const TITLE_H = 36
  const STAT_H = 24
  ...
  const hasMatDesc = !!(spell.components.material && spell.components.materialDesc.trim())
  ...
  return (
    ...
    >
      ...
      {/* Outer border */}
      <rect
        x="1.5" y="1.5" width={CW - 3} height={CH - 3}
        fill="none" stroke="#1a1208" strokeWidth="3" rx="1"
      />

      {/* Inner thin border */}
      <rect
        x="5" y="5" width={CW - 10} height={CH - 10}
        fill="none" stroke="#1a1208" strokeWidth="0.5"
      />

      {/* Title block */}
      <rect x="1.5" y="1.5" width={CW - 3} height={TITLE_H} fill="#1a1208" rx="1" />
      ...
      <text
        ...
        fontSize="11.5" fontWeight="700" fontFamily={ff} fill="white"
      >
        {spell.name}
      </text>
      ...
      <text
        x={MX} y={TITLE_H * 0.79}
        textAnchor="middle" dominantBaseline="middle"
        fontSize="6.5" fontStyle="italic" fontFamily={ff} fill="#c4bbb0"
      >
        {levelLabel(spell.level, spell.school)}
      </text>

      {/* ── Stat row 1: Casting Time | Range ── */}
      <text x={PAD} y={STAT1_Y + 6} dominantBaseline="hanging"
        fontSize="4.5" fontWeight="700" fontFamily={ff} fill="#6a5a48" letterSpacing="0.4">
        CASTING TIME
      </text>
      <text x={PAD} y={STAT1_Y + 14} dominantBaseline="hanging"
        fontSize="8" fontFamily={ff} fill="#1a1208">
        {spell.castingTime}
      </text>

      <line x1={MX} y1={STAT1_Y + 3} x2={MX} y2={STAT1_Y + STAT_H - 3}
        stroke="#1a1208" strokeWidth="0.5" opacity="0.35" />

      <text x={MX + 4} y={STAT1_Y + 6} dominantBaseline="hanging"
        fontSize="4.5" fontWeight="700" fontFamily={ff} fill="#6a5a48" letterSpacing="0.4">
        RANGE
      </text>
      <text x={MX + 4} y={STAT1_Y + 14} dominantBaseline="hanging"
        fontSize="8" fontFamily={ff} fill="#1a1208">
        {spell.range}
      </text>

      <line x1={PAD} y1={STAT1_Y + STAT_H} x2={CW - PAD} y2={STAT1_Y + STAT_H}
        stroke="#1a1208" strokeWidth="0.5" opacity="0.35" />

      {/* ── Stat row 2: Components | Duration ── */}
      <text x={PAD} y={STAT2_Y + 6} dominantBaseline="hanging"
        fontSize="4.5" fontWeight="700" fontFamily={ff} fill="#6a5a48" letterSpacing="0.4">
        COMPONENTS
      </text>
      <text x={PAD} y={STAT2_Y + 14} dominantBaseline="hanging"
        fontSize="8" fontFamily={ff} fill="#1a1208">
        {componentStr(spell.components)}
      </text>

      <line x1={MX} y1={STAT2_Y + 3} x2={MX} y2={STAT2_Y + STAT_H - 3}
        stroke="#1a1208" strokeWidth="0.5" opacity="0.35" />

      <text x={MX + 4} y={STAT2_Y + 6} dominantBaseline="hanging"
        fontSize="4.5" fontWeight="700" fontFamily={ff} fill="#6a5a48" letterSpacing="0.4">
        DURATION
        {spell.tags.concentration && (
          <tspan fontSize="4" fill="#7a4a18"> ◉ C</tspan>
        )}
      </text>
      <text x={MX + 4} y={STAT2_Y + 14} dominantBaseline="hanging"
        fontSize="8" fontFamily={ff} fill="#1a1208">
        {spell.duration}
      </text>

      <line x1={PAD} y1={DIV2_Y} x2={CW - PAD} y2={DIV2_Y}
        stroke="#1a1208" strokeWidth="0.5" opacity="0.35" />

      {/* ── Description text ── */}
      {textLines.map((line, i) => (
        <text key={i} x={PAD} y={line.y}
          fontSize={FONT_SIZE} fontFamily={ff} fill="#1a1208">
          {line.text}
        </text>
      ))}
      ...
      {hasMatDesc && (
        <>
          <line x1={PAD} y1={COMP_Y - 3} x2={CW - PAD} y2={COMP_Y - 3}
            stroke="#1a1208" strokeWidth="0.5" opacity="0.35" />
          {/* Diamond icon */}
          <polygon
          ...
          {matLines.map((line, i) => (
            <text
              ...
              x={PAD + 11}
              y={COMP_Y + 5.5 + i * 7.5}
              fontSize="5.5" fontFamily={ff} fill="#1a1208"
            >
              {line}
            </text>
          ))}
      ...
      {/* ── Footer ── */}
      <line x1={PAD} y1={FOOTER_Y} x2={CW - PAD} y2={FOOTER_Y}
        stroke="#1a1208" strokeWidth="0.5" opacity="0.35" />

      <text
        x={MX} y={FOOTER_Y + FOOTER_H / 2}
        textAnchor="middle" dominantBaseline="middle"
        fontSize="5" fontFamily={ff} fill="#6a5a48" fontStyle="italic"
      >
        {footerText}
      </text>
    </svg>
...
export function SpellCardWidget() {
  return <SpellCardSvg spell={DANCING_LIGHTS} />
}
