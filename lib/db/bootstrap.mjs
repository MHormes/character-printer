...
import { Pool } from "pg";
import { S3Client, ListObjectsV2Command, PutObjectCommand, CreateBucketCommand } from "@aws-sdk/client-s3";
import { seedSrdPostgres } from "./seed-srd-postgres.mjs";
import { seedSrd2024Postgres } from "./seed-srd-2024-postgres.mjs";
...
async function shouldSeedSrd(client) {
  const r2014 = await client.query("SELECT COUNT(*)::int AS count FROM backgrounds WHERE system = $1", ["dnd5e"]);
  const r2024 = await client.query("SELECT COUNT(*)::int AS count FROM backgrounds WHERE system = $1", ["dnd5e_2024"]);
  return r2014.rows[0]?.count === 0 || r2024.rows[0]?.count === 0;
}
...
async function seedStorage() {
  if (!fs.existsSync(storageDir)) {
    console.log("No database/storage mount found. Skipping image seed.");
    return;
  }
  ...
  if (imageFiles.length === 0) {
    console.log("database/storage is empty. Skipping image seed.");
    return;
  }
...
async function main() {
  ...
  try {
    ...
    const csvFound = hasCsvData(dataDir);
    console.log(`[bootstrap] dataDir=${dataDir} exists=${fs.existsSync(dataDir)} users.csv=${csvFound}`);
    if (csvFound && await hasExistingData(client)) {
      console.log("DB already populated. Skipping CSV import.");
    } else if (csvFound) {
      ...
      console.log("SRD data missing. Seeding 2014 and 2024 SRD data from network...");
      await seedSrdPostgres(client);
      await seedSrd2024Postgres(client);
    } else {
